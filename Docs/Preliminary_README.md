...
This README describes the parameter CSV and the companion Python GUI/visualiser for the symmetric belted NEMA17 actuator layout.
...
So the output shaft is the origin. The motor sits above it. The two idlers sit symmetrically left and right. The GUI solves `idler_y_nominal_mm` from the chosen belt length, then applies `tension_offset_mm` to get the current `idler_y_mm`.
...
The solver can return zero, one, or multiple valid Y positions. If there are multiple candidates, the GUI chooses the best-scored one based on wrap and compactness and reports the candidate count.
...
## Outputs generated by the solver / GUI

The Python tool and the parameter file are intended to generate these outputs:
...
| `belt_residual_mm` in GUI | Difference between modelled belt length and selected belt length at the current tension offset. Ideally near zero at zero offset. |
...
## What the Python GUI does

The Python script is intended as a layout solver and visual checker before committing the geometry to Fusion 360.

It provides:

- Sliders and entry fields for the main input parameters.
- A live 2D Matplotlib drawing of the actuator.
...
The GUI is not meant to be a replacement for CAD. It is a fast pre-layout tool. Fusion 360 is still where the actual plates, bearing pockets, pulley models, motor mount, slots, spacers, shafts, and output structure should be designed.
...
1. Use the Python GUI to solve `idler_y_nominal_mm`.
2. Export the CSV.
...
6. Use the GUI to solve `idler_y_nominal_mm`.
...
`Python GUI → solve layout → export CSV → Fusion 360 user parameters → parametric CAD model`
