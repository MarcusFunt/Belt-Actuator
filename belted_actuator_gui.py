...
- Matplotlib 2D visualiser with belt path, pulley/idler circles, and dimension lines
- Solves neutral idler Y from fixed belt length using a tangent-belt geometry model
- Solves neutral idler X from fixed belt length (alternative solve mode)
...
Notes
- Pulleys are modelled at pitch radius.
- Smooth idlers are modelled at idlerOD/2 + beltBackToPitch for this 2D layout.
...
class ParamControl:
    """A labelled slider + text-entry pair that stays in sync."""
    ...
    def __init__(
        ...
        self.scale = ttk.Scale(
            parent, from_=low, to=high, variable=self.var,
            command=lambda _=None: self._scale_changed(callback),
        )
    ...
    def _scale_changed(self, callback):
        value = self.var.get()
        value = round(value) if self.step >= 1 else round(value / self.step) * self.step
        self.var.set(value)
...
class ActuatorGUI(tk.Tk):
    ...
    def _build_ui(self):
        ...
        ttk.Label(left, text="Input parameters", font=("Segoe UI", 12, "bold")).grid(
        ...
        # (frame, start_row, [(name, label, unit, initial, low, high, step), ...])
        control_groups = [
            (drive_frame, 1, [
        ...
        solver_inputs = {
            "belt_pitch_mm", "belt_length_mm",
            "pulleyI_teeth", "pulleyO_teeth",
            "idler_OD_mm", "center_IO_mm", "idler_x_offset_mm",
        }
        ...
        self.readout = tk.Text(
            diagnostics, height=10, width=64, wrap="word",
        ...
        ttk.Label(root, textvariable=self.status_var, foreground="#444", anchor="w").grid(
    ...
    def solve_idler_y(self, show_errors: bool = True, update_status: bool = True) -> Optional[float]:
        p = self.params()
    ...
    def dim_line(self, ax, p1: Point, p2: Point, text: str, offset: Point = (0, 0)):
        """Draw a dimension annotation between two points."""
        x1, y1 = p1
        x2, y2 = p2
        ox, oy = offset
        a = (x1 + ox, y1 + oy)
        ...
        ax.text(
            mid[0], mid[1], text, fontsize=8, ha="center", va="center",
    ...
    def redraw(self):
        ...
        if sol.valid and self.layout_error is None:
            ...
            for pts in sol.arc_points.values():
                ax.plot([q[0] for q in pts], [q[1] for q in pts], lw=belt_lw, color="black", solid_capstyle="round")
        else:
        ...
        if self.solved_idler_y_nominal is not None and p["tension_slot_travel_mm"] > 0:
            ...
            for x in (-model_p["idlerX"], model_p["idlerX"]):
                ax.plot(
                    ...
                    color="#1f77b4", lw=1.4, alpha=0.75,
                )

        # Pulley and idler circles.
        for c in circles:
            patch = ax.add_patch(MplCircle(c.center, c.radius, fill=False, lw=2.0, linestyle="-"))
            if any(c.name in warning for warning in d["clearanceWarnings"]):
                patch.set_edgecolor("red")
            ax.plot(c.center[0], c.center[1], marker="+", ms=9)
            ax.text(
                c.center[0], c.center[1] + c.radius + 3, c.name,
        ...
        ax.plot(
            [-model_p["idlerX"], model_p["idlerX"]],
            [model_p["idlerY"], model_p["idlerY"]],
        ...
        self.dim_line(ax, (0, 0), (0, model_p["motorY"]),
                      f"center_IO {model_p['motorY']:.1f}", offset=(x_pad, 0))
        self.dim_line(ax, (-model_p["idlerX"], model_p["idlerY"]), (model_p["idlerX"], model_p["idlerY"]),
                      f"idler span {2*model_p['idlerX']:.1f}", offset=(0, 12))
        self.dim_line(ax, (0, 0), (model_p["idlerX"], model_p["idlerY"]),
                      f"O-idler {d['centerOIdler']:.1f}", offset=(8, -8))
        self.dim_line(ax, (0, model_p["motorY"]), (model_p["idlerX"], model_p["idlerY"]),
                      f"I-idler {d['centerIIdler']:.1f}", offset=(8, 8))

        # Axis limits.
        margin = 35
        slot_y_min = (self.solved_idler_y_nominal - p["tension_slot_travel_mm"] / 2
                      if self.solved_idler_y_nominal is not None else model_p["idlerY"])
        slot_y_max = (self.solved_idler_y_nominal + p["tension_slot_travel_mm"] / 2
                      if self.solved_idler_y_nominal is not None else model_p["idlerY"])
        ax.set_xlim(
            min(-model_p["idlerX"] - d["idlerEffR"], -d["pulleyOPitchR"], -d["pulleyIPitchR"]) - margin,
            max(model_p["idlerX"] + d["idlerEffR"], d["pulleyOPitchR"], d["pulleyIPitchR"], x_pad + 5) + margin,
        )
        ax.set_ylim(
            min(-d["pulleyOPitchR"], model_p["idlerY"] - d["idlerEffR"], slot_y_min) - margin,
            max(model_p["motorY"] + d["pulleyIPitchR"], model_p["idlerY"] + d["idlerEffR"], slot_y_max, y_pad) + margin,
        )
    ...
    def update_readout(self, p: Dict[str, float], d: Dict, sol: BeltSolution):
        ...
        offset_warning = (
            abs(p["tension_offset_mm"]) > travel_half + 1e-9
            if p["tension_slot_travel_mm"] >= 0
            else False
        )
        ...
        if self.layout_error is not None:
            ...
            lines += ["Wrap angles:"] + [f"  {k}: {v:.1f} deg" for k, v in sol.wrap_deg.items()]
        else:
    ...
    def fusion_rows(self) -> List[Tuple[str, str, str, str]]:
        ...
        belt_type_comment = f"GUI belt profile: {belt_type}. 0=Custom, 1=GT2, 2=GT3, 3=HTD-3M, 4=HTD-5M."
    ...
    def export_csv_dialog(self):
        path = filedialog.asksaveasfilename(
            ...
            title="Export Fusion 360 parameters",
        )
        if not path:
            return
        rows = self.fusion_rows()
        try:
            with open(path, "w", newline="", encoding="utf-8") as f:
                writer = csv.writer(f)
                writer.writerow(["Name", "Unit", "Expression", "Comment"])
                writer.writerows(rows)
            messagebox.showinfo("Exported", f"Parameters saved to:\n{path}")
